...
let currentIndex = -1;
let currentImgs = []; // lista de <img> (tanto do grid quanto dos previews)

function getFullSrc(imgEl) {
  return imgEl?.dataset?.full || imgEl?.src || "";
}

function openLightboxWithList(imgList, startIndex) {
  currentImgs = Array.from(imgList || []).filter(Boolean);
  if (!currentImgs.length) return;

  currentIndex = Math.max(0, Math.min(startIndex, currentImgs.length - 1));
  lbImg.src = getFullSrc(currentImgs[currentIndex]);
...
const gridImgs = gridItems.map(it => it.querySelector('img')).filter(Boolean);

gridItems.forEach((item, idx) => {
  item.addEventListener('click', () => openLightboxWithList(gridImgs, idx));
});

// ================== ABRIR LIGHTBOX: EVENTOS (PREVIEW) ==================
document.querySelectorAll('.event-preview').forEach(preview => {
  const imgs = preview.querySelectorAll('img');
  imgs.forEach((img, idx) => {
    img.addEventListener('click', (e) => {
      e.stopPropagation();
      openLightboxWithList(imgs, idx);
    });
  });
});
...
// Eventos: abre/fecha preview de 2 fotos + abre no lightbox
document.querySelectorAll(".event-portfolio").forEach(card => {
  const btn = card.querySelector(".event-open");
  ...
  preview.querySelectorAll("img").forEach(img => {
    img.addEventListener("click", (e) => {
      ...
      // reaproveita seu lightbox já existente
      const lb = document.getElementById("lightbox");
      const lbImg = document.getElementById("lightboxImg");
      if (!lb || !lbImg) return;
...
(function atualizarDisponibilidadeMeses() {
  const elPoucas = document.getElementById("mesPoucas");
  const elAinda = document.getElementById("mesAinda");
  if (!elPoucas || !elAinda) return;
  ...
  const mes1 = addMonths(now, 1); // mês seguinte
  const mes2 = addMonths(now, 2); // mês depois do próximo

  const fmtMes = new Intl.DateTimeFormat("pt-BR", { month: "long" });

  function formatarMes(d) {
    const mes = fmtMes.format(d);
    const mesCap = mes.charAt(0).toUpperCase() + mes.slice(1);
    const ano = d.getFullYear();

    // só mostra o ano se for diferente do atual
    return ano !== anoAtual ? `${mesCap}/${ano}` : mesCap;
  }

  elPoucas.textContent = formatarMes(mes1);
  elAinda.textContent  = formatarMes(mes2);
})();
